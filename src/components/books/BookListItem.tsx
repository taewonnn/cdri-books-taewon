...
export default function BookListItem({ book }: { book: Book }) {
  ...
  return (
    <li className="border-b border-gray last:border-b-0">
      {/* 접힌 상태 */}
      {!open ? (
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 p-4 sm:px-12 sm:h-25">
          {/* 썸네일 */}
          <div className="relative shrink-0 w-16 h-22 sm:w-12 sm:h-17 rounded overflow-hidden bg-lightGray">
            <Img
          ...
          <div className="min-w-0 flex-1 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            <h3 className="font-bold text-base sm:text-lg text-primary leading-tight truncate">{book.title}</h3>
          ...
          <div className="w-full sm:w-auto shrink-0 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center justify-center h-11 sm:h-12 px-6 sm:px-7 rounded-lg bg-primary text-white text-sm sm:text-base font-medium"
            >
              구매하기
            </a>
            ...
            <button
              ...
              className="inline-flex items-center justify-center h-11 sm:h-12 px-4 rounded-lg bg-lightGray text-secondary text-sm sm:text-base font-medium"
              aria-expanded={open}
            >
              상세보기
        ...
        // 펼친 상태
        <div className="p-4 sm:p-8 lg:px-12 lg:py-10 flex flex-col lg:flex-row gap-6 lg:gap-10 bg-white">
          <div className="relative shrink-0 w-full max-w-[320px] sm:max-w-[360px] lg:w-[210px] lg:h-[280px] aspect-3/4 rounded overflow-hidden bg-lightGray">
            <Img
              src={book.thumbnail || bookPlaceholder}
              alt={book.title}
              width={210}
          ...
          {/* 오른쪽 가격/버튼 영역 */}
          <div className="shrink-0 w-full lg:w-60 flex flex-col items-stretch lg:items-end gap-3">
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="inline-flex items-center justify-center h-11 sm:h-12 px-4 sm:px-5 rounded-lg bg-lightGray text-secondary text-sm sm:text-base font-medium w-full lg:w-auto"
              aria-expanded={open}
            >
              상세보기
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="lg:mt-6 inline-flex items-center justify-center w-full h-12 sm:h-14 rounded-xl bg-primary text-white text-sm sm:text-base font-semibold hover:opacity-90 transition-opacity"
            >
              구매하기
            </a>
          </div>
